"""
Multi round dialogue with langgraph, support automatic memory and multiprocess conversation, and with customized prompt template and trimmed context length.
Reference: https://python.langchain.com/docs/tutorials/chatbot/
Dependencies:
```
pip install langchain-core, langgraph>0.2.27
pip install -qU langchain-mistralai
pip install transformers    # need tokenizer to trim the context
```
"""
import os
...
def call_model(state: MessagesState):
    trimmed_messages = trimmer.invoke(state["messages"])    # trim the context
    prompt = prompt_template.invoke({"messages": trimmed_messages})  # use prompt_template.invoke() to incorporate the template
